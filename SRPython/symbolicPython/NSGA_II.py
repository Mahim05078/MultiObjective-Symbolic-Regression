...
class NSGA_II:
	...
	def Run(self):
		...
		for i in range( self.pop_size ):
			...
			t = Operators.GenerateRandomTree( self.functions, self.terminals, curr_max_depth, curr_height=0, 
		...
		while not self.__ShouldTerminate():
			...
			O = []  # candidate population for the next gen
			for i in range( self.pop_size ):
				o = deepcopy(selected[i])
				if ( random() < self.crossover_rate ):
					o = Operators.SubtreeCrossover( o, selected[ randint( self.pop_size ) ] )
				if ( random() < self.mutation_rate ):
					o = Operators.SubtreeMutation( o, self.functions, self.terminals, max_height=self.initialization_max_tree_height )
				if ( random() < self.op_mutation_rate ):
					o = Operators.OnePointMutation( o, self.functions, self.terminals )
			...
			while curr_front_idx < len(fronts) and len(fronts[curr_front_idx]) + len(new_population) <= self.pop_size:
				self.ComputeSparsity( fronts[curr_front_idx] )
	...
	def ComputeSparsity(self, front):
